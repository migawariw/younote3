...
async function loadMemos(){
  ...
  snap.forEach(d=>{
    ...
    const dt=new Date(data.updated); dateSpan.textContent=dt.toLocaleString();
    const delBtn=document.createElement('button'); delBtn.className='delete-btn'; delBtn.textContent='🗑';
    delBtn.onclick=async(e)=>{ e.stopPropagation(); await deleteDoc(doc(db,'users',auth.currentUser.uid,'memos',d.id)); loadMemos();}
...
document.addEventListener('click',(e)=>{
  if(!e.target.closest('#editor')&&!e.target.closest('#title')) document.activeElement.blur();
});
...
editor.addEventListener('paste', async(e)=>{
  ...
  if(yt){
    ...
    if(m){
      const img=document.createElement('img'); img.src=m[0]; range.insertNode(img); range.collapse(false);
    } else {
